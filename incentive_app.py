...
def load_structure_dump(uploaded_file):
    """
    Load employee master from Structure Dump Excel.
    Returns a DataFrame with normalised columns the app understands.

    Mapping from Structure Dump → App fields:
      Employee ID        → emp_id
      Employee Name      → emp_name
      IIL Vertical Name  → vertical  (CSD / KCD)
      Joining Date       → joining_date
      Location           → location
      Final Group        → vintage   (0-30D / 31-90D / 91-270D / 270D+)
      Vintage Bucket     → vintage_bucket (SPS / 0-90 Days / Delhi KCD / Vashi KCD / etc.)
      Remarks            → team_type  (Listing / Catalog / -)
      Client-C           → client_count_csd  (calculated clients, CSD)
      Client-A           → client_count_kcd  (actual clients, KCD)
      L2/L3/L4/L5 Name   → hierarchy
    """
    if uploaded_file is None:
        return pd.DataFrame()
    ...
    # Resolve each field
    emp_id_col    = fc(["Employee ID", "EmpID", "Emp ID"])
    name_col      = fc(["Employee Name", "Name"])
    vertical_col  = fc(["IIL Vertical Name", "Vertical", "VERTICAL"])
    joining_col   = fc(["Joining Date", "DOJ", "Date of Joining"])
    location_col  = fc(["Location", "LOCATION"])
    vintage_col   = fc(["Final Group", "Vintage", "VINTAGE"])
    vbucket_col   = fc(["Vintage Bucket", "VintageBucket"])
    remarks_col   = fc(["Remarks", "REMARKS"])
    client_c_col  = fc(["Client-C", "ClientC", "Calculated Clients"])
    client_a_col  = fc(["Client-A", "ClientA", "Actual Clients"])
    l2_name_col   = fc(["L2 Name", "L2Name", "Manager"])
    l3_name_col   = fc(["L3 Name", "L3Name", "HOD - 1", "HOD-1"])
    l4_name_col   = fc(["L4 Name", "L4Name", "HOD"])
    l5_name_col   = fc(["L5 Name", "L5Name"])
    date_left_col = fc(["Date Left", "DateLeft", "Exit Date"])
    designation_col = fc(["Designation", "DESIGNATION", "Grade", "GRADE"])

    out = pd.DataFrame()
    out["Employee ID"]    = df[emp_id_col].astype(str).str.strip() if emp_id_col else ""
    out["Employee Name"]  = df[name_col].fillna("") if name_col else ""
    out["Vertical"]       = df[vertical_col].fillna("").astype(str).str.upper().str.strip() if vertical_col else ""
    out["Joining Date"]   = df[joining_col] if joining_col else None
    out["Location"]       = df[location_col].fillna("").astype(str) if location_col else ""
    out["Vintage"]        = df[vintage_col].fillna("").astype(str).str.strip() if vintage_col else ""
    out["Vintage Bucket"] = df[vbucket_col].fillna("").astype(str).str.strip() if vbucket_col else ""
    out["Remarks"]        = df[remarks_col].fillna("").astype(str).str.strip() if remarks_col else ""
    out["Client-C"]       = pd.to_numeric(df[client_c_col], errors="coerce").fillna(100) if client_c_col else 100
    out["Client-A"]       = pd.to_numeric(df[client_a_col], errors="coerce").fillna(100) if client_a_col else 100
    out["L2 Name"]        = df[l2_name_col].fillna("") if l2_name_col else ""
    out["L3 Name"]        = df[l3_name_col].fillna("") if l3_name_col else ""
    out["L4 Name"]        = df[l4_name_col].fillna("") if l4_name_col else ""
    out["L5 Name"]        = df[l5_name_col].fillna("") if l5_name_col else ""
    out["Date Left"]      = df[date_left_col] if date_left_col else None
    out["Designation"]    = df[designation_col].fillna("") if designation_col else ""

    # Derive team from Vintage Bucket + Remarks + Vertical
    def derive_team(row):
        vb  = str(row["Vintage Bucket"]).upper()
        rem = str(row["Remarks"]).upper()
        vert= str(row["Vertical"]).upper()
        vintage = str(row["Vintage"]).upper()

        if "CSD" in vert:
            if vintage in ("0-30D",):
                return "0-90 Days (CSD new)"
            elif vintage in ("31-90D",):
                return "0-90 Days (CSD new)"
            elif "SPS" in vb or "90+" in vb:
                return "SPS (CSD 91D+)"
            elif "ROI" in vb:
                return "ROI CSD"
            else:
            ...
            if "LISTING" in rem:
                ...
                return "Catalog (KCD)"
            elif "NAGPUR" in vb or "NAGPUR" in str(row["Location"]).upper():
                return "Nagpur Pharma KCD"
            elif any(c in str(row["Location"]).upper()
                     for c in ["HYDERABAD","VASHI","RAIPUR","INDORE"]):
                return "HVRI KCD"
            elif "ROI" in vb:
                return "ROI KCD"
            else:
    ...
    out["Team"] = out.apply(derive_team, axis=1)

    # Client Count: CSD uses Client-C (calculated, min 50), KCD uses Client-A (actual)
    out["Client Count"] = out.apply(
        lambda r: max(float(r["Client-C"]), 50) if "CSD" in str(r["Vertical"]).upper()
                  else float(r["Client-A"]),
        axis=1
    )
