...
def clean_receipt(df):
    """Keep CLEARED + PENDING rows (exclude Bounced/Cancelled).
    Matches FSF app: Pending rows are included as they haven't bounced yet."""
    df = df.copy()
    bc = find_col(df, ["B/C"])
    if bc:
    ...
    if sc:
        ...
        valid = df[_sv.isin(["CLEARED","PENDING"])]
