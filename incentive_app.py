...
def enrich_receipt_data(rec_df, structure_result=None):
    ...
    # Step 1: flag upsell rows (Upsell/Unique col non-blank)
    if upsell_col_e:
        df["_is_upsell"] = df[upsell_col_e].apply(lambda x: _str_e(x) != "")
    else:
        df["_is_upsell"] = False

    # Step 2: flag pure renewal rows (product name is a renewal-only product)
    _PURE_RENEWAL_PRODUCTS = {
    ...
    if prod_c:
        df["_is_pure_renewal"] = df[prod_c].apply(
            lambda x: _str_e(x) in _PURE_RENEWAL_PRODUCTS)
    else:
        df["_is_pure_renewal"] = False

    # Step 3: set of receipt IDs that have a REAL upsell (WT AMT > 0)
    wt_col_e = find_col(df, ["WT AMT", "WT_AMT", "WTAMT"])
    if rcpt_id_e and wt_col_e:
        real_upsell_mask = df["_is_upsell"].astype(bool) & (df[wt_col_e].fillna(0) > 0)
        real_upsell_ids  = set(df.loc[real_upsell_mask, rcpt_id_e].tolist())
        df["_has_upsell_on_receipt"] = df[rcpt_id_e].isin(real_upsell_ids)
    elif rcpt_id_e:
        upsell_ids = set(df.loc[df["_is_upsell"].astype(bool), rcpt_id_e].tolist())
        df["_has_upsell_on_receipt"] = df[rcpt_id_e].isin(upsell_ids)
    else:
        df["_has_upsell_on_receipt"] = df["_is_upsell"].astype(bool)

    # Step 4: Retention rows are NOT productive
    _rnl_rem_col_e = find_col(df, ["Rnl Remarks", "RnlRemarks", "Renewal Remarks"])
    _is_retention = pd.Series(False, index=df.index)
    if _rnl_rem_col_e:
        _is_retention = df[_rnl_rem_col_e].astype(str).str.strip().str.lower() == "retention"

    is_upsell_e       = df["_is_upsell"].astype(bool)
    is_pure_renewal_e = df["_is_pure_renewal"].astype(bool)
    has_upsell_e      = df["_has_upsell_on_receipt"].astype(bool)

    productivity = (
        (is_upsell_e | (is_pure_renewal_e & ~has_upsell_e)) & ~_is_retention
    ).astype(float)

    # Step 5: Insta products → 0.5 productivity
    _INSTA_PRODUCTS_E = {"IM InstaDiamond", "IM InstaGold", "IM InstaPlatinum",
                         "IM insta Diamond", "IM Insta Renewal",
                         "Lead Manager Pro Gold", "Lead Manager Pro Platinum"}
...
def clean_receipt(df):
    """Keep CLEARED + PENDING + blank-Status rows (exclude Bounced/Cancelled B/C).
    - Blank Status rows (e.g. Refund): included for completeness, Productivity=0
    - Bounced/Cancelled (B/C col): always excluded
    Matches FSF app behaviour."""
    df = df.copy()
    bc = find_col(df, ["B/C"])
    if bc:
        _bc_vals = df[bc].astype(str).str.strip().str.upper()
        df = df[~_bc_vals.isin(["BOUNCED","CANCELLED","CANCELED","BOUNCE","CANCEL"])]
    sc = find_col(df, ["Status","PAYMENT STATUS","Payment Status"])
    if sc:
        _sv = df[sc].astype(str).str.upper().str.strip()
        # Keep Cleared, Pending, AND blank/NaN Status rows
        valid = df[_sv.isin(["CLEARED","PENDING"]) | df[sc].isna() | (_sv == "NAN")]
        if len(valid) > 0: df = valid
    return df
...
# Load files
rec_raw_full = _rf(receipt_f)                  # full unfiltered — used for enriched receipt output
rec_raw      = clean_receipt(rec_raw_full)      # clean filtered — used for incentive calculation
